...
const defaultPosts: {
...
}[] = [
  {
    ...
    excerptEn:
      "Discover the best places to visit in Jeddah, from the historic Al-Balad to the Red Sea coast and modern landmarks.",
    excerptAr:
      "اكتشف أفضل الأماكن للزيارة في جدة، من البلاد التاريخي إلى ساحل البحر الأحمر والمعالم الحديثة.",
  ...
  },
  {
    ...
    excerptEn:
      "From historic Diriyah to the modern skyline, explore the capital's blend of tradition and innovation.",
    excerptAr:
      "من الدرعية التاريخية إلى الأفق الحديث، استكشف مزيج العاصمة من التقاليد والابتكار.",
  ...
  },
  {
    ...
    titleAr: "تجربة العلا",
    excerptEn:
      "Ancient tombs, dramatic landscapes, and cultural events in one of the world's most remarkable destinations.",
    excerptAr:
      "مقابر قديمة ومناظر طبيعية درامية وفعاليات ثقافية في واحدة من أبرز الوجهات في العالم.",
      ...
      ? posts.slice(0, 3).map((p, i) => ({
          ...
          excerpt:
            locale === "ar"
              ...
              : p.excerpt_en || p.excerpt_ar || "",
          image:
            p.featured_image ||
            "https://images.unsplash.com/photo-1540331547164-8b63109225b7?w=600&q=80",
          category: p.category || (locale === "ar" ? "مقالات" : "Articles"),
          date: p.published_at
            ? formatDate(p.published_at.toString(), locale)
            : "",
          ...
          {items.map((post, index) => (
            ...
            >
              ...
              >
                ...
                {post.date && (
                  <p className="text-xs text-white/80 mt-auto">{post.date}</p>
                )}
              </div>
            </motion.article>
          ))}
